...
func (s *SwellDetector) Observe(level float64) (bool, model.GlycolCondition) {
	delta := level - s.lastLevel
	s.lastLevel = level
	if delta > 5 {
		s.lastChangeAt = s.clk.Now()
		return true, model.GlycolSwell
	}
	if delta < -5 {
		s.lastChangeAt = s.clk.Now()
		return true, model.GlycolShrink
	}
	return false, model.GlycolNormal
}
...
func (s *SwellDetector) ShrinkRecoveryAdvice(cond model.GlycolCondition) string {
	if cond == model.GlycolShrink {
